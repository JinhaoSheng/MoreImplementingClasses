...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        h_upper_left_corner = rg.Point(intersection_center.x - width/2,
                                       intersection_center.y - letter_thickness/2)
        h_lower_right_corner = rg.Point(intersection_center.x + width/2,
                                        intersection_center.y + letter_thickness/2)
        v_upper_left_corner = rg.Point(intersection_center.x - letter_thickness/2,
                                       intersection_center.y - letter_thickness/2)
        v_lower_right_corner = rg.Point(intersection_center.x + letter_thickness/2,
                                        intersection_center.y - letter_thickness/2 + height)
        self.h_rect = rg.Rectangle(h_upper_left_corner, h_lower_right_corner)
        self.v_rect = rg.Rectangle(v_upper_left_corner, v_lower_right_corner)
